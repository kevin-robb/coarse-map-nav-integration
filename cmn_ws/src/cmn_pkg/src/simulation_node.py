...
def get_occ_map(msg):
    ...
    # Set map in the simulator class, which will keep track of the ground truth vehicle pose and generate ground-truth observations from it.
    sim.set_map(occ_map_true)
    # Add the full map to our visualization (after it's processed).
    ax0.imshow(sim.map, cmap="gray", vmin=0, vmax=1)
    """
...
############################ SIMULATOR FUNCTIONS ####################################
def update_plot(obs_img, rect):
    """
    Update the visualization.
    @param obs_img, ground-truth observation.
    ...
    veh_row_est, veh_col_est = sim.transform_map_m_to_px(veh_pose_est[0], veh_pose_est[1])
    ...
    if particles_x is not None:
        ...
        for i in range(g_pf_num_particles):
            r, c = sim.transform_map_m_to_px(particles_x[i], particles_y[i])
    ...
    if "veh_pose_obs" not in plots.keys():
        plots["veh_pose_obs"] = ax1.arrow(sim.veh_px_vert_from_bottom_on_obs-0.5, sim.obs_width_px // 2 + sim.veh_px_horz_from_center_on_obs, 0.5*sim.obs_resolution, 0.0, color="blue", width=0.01/sim.obs_resolution/sim.map_downscale_ratio, zorder = 2)
...
def generate_observation():
    ...
    obs_img, rect = sim.get_true_observation()

    # Update the visualization.
    update_plot(obs_img, rect)
...
def main():
    ...
    rospy.Subscriber(g_topic_commands, Twist, get_vel_command, queue_size=1)
